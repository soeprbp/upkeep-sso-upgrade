...
export function normalizeEntraUser(user) {
  ...
  const rawGroups = user.adGroups ?? user.memberOf ?? user.groups ?? "";
  ...
  return {
    ...
    office: user.office ?? user.physicalDeliveryOfficeName ?? "",
...
export async function readEntraUsersFromCsv(filePath) {
  ...
  return parseCsv(text).map((row) =>
    normalizeEntraUser({
      ...
      employeeId: row.employeeId ?? row.EmployeeId ?? row.employeeID ?? row.EmployeeID,
      manager: row.manager ?? row.Manager,
      adGroups: row.adGroups ?? row.ADGroups,
      memberOf: row.memberOf ?? row.MemberOf
    })
  );
}
