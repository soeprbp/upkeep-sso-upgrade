...
export function parseUpKeepSites() {
  ...
  if (sitesVar) {
    ...
    const includeDemoSite =
      String(process.env.UPKEEP_INCLUDE_DEMO_SITE ?? "").trim().toLowerCase() === "true";
    return includeDemoSite
      ? sites
      : sites.filter((site) => site.name.trim().toLowerCase() !== "demo site");
  }
