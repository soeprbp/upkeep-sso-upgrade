...
export function diffUsers(upkeepUsers, entraUsers, payrollUsers = [], options = {}) {
  ...
  const payrollChecked = options.payrollChecked ?? payrollUsers.length > 0;

  return upkeepUsers.map((upkeepUser) => {
    const entraUser = upkeepUser.email ? entraByEmail.get(upkeepUser.email) : null;
    const payrollUser = findPayrollUser(upkeepUser, payrollIndex);
    const payrollMatchStatus = payrollUser
      ? payrollUser.active
        ? "active"
        : "inactive"
      : payrollChecked
        ? "not_found"
        : "not_checked";
    const hasMissingPayroll = payrollMatchStatus === "not_found";
    const hasDisabledAd = Boolean(entraUser && !entraUser.accountEnabled);
    let status = "matched";
    let action = "No action needed";

    if (payrollUser && !payrollUser.active) {
      status = "terminated_payroll_user";
      action = "Disable or remove this UpKeep account; payroll status is inactive/terminated";
    } else if (!upkeepUser.email) {
      status = "missing_upkeep_email";
      action = "Add or correct the UpKeep email before SSO cutover";
    } else if (!payrollChecked) {
      status = "payroll_not_checked";
      action = "Run the payroll reconciliation before approving this account for SSO";
    } else if (hasMissingPayroll) {
      status = "missing_payroll_user";
      action = "Verify employment status or remove this account from the SSO rollout";
    } else if (!entraUser) {
    ...
    return {
      ...
      payrollStatus: payrollUser?.raw?.status ?? payrollUser?.raw?.Status ?? "",
